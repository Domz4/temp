...
myKeys conf@(XConfig{XMonad.modMask = modm}) =
  M.fromList
    ...
    , -- close focused window , ((modm .|. shiftMask, xK_c     ), kill)
      -- GAPS!!!
...
myStartupHook = do
  spawnOnce "xset r rate 175 50"
  spawnOnce "exec ~/bin/bartoggle"
  spawnOnce "exec ~/bin/eww daemon"
  spawnOnce "feh --bg-scale ~/wallpapers/we-must-conquer-mars-wallpaper.jpg"
...
defaults =
  def
    ...
    , layoutHook = gaps [(L, 3), (R, 3), (U, 25), (D, 3)] $ spacingRaw True (Border 1 1 1 1) True (Border 1 1 1 1) True $ smartBorders $ myLayout
